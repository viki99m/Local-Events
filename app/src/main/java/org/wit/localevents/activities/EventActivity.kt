...
class EventActivity : AppCompatActivity(), DatePickerDialog.OnDateSetListener,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.toolbarAdd.title = resources.getString(R.string.app_name)
        ...
        if (intent.hasExtra("event_edit")) {
            ...
            binding.showDate.text =
                "${event.date.year} - ${event.date.month} - ${event.date.dayOfMonth}, ${
                    format.format(event.date.hour)
                }:${format.format(event.date.minute)}"
        ...
        binding.buttonAddEvent.setOnClickListener {
            ...
            if (event.name.isEmpty()) {
                Snackbar.make(it, R.string.hint_event_name, Snackbar.LENGTH_LONG)
                    .show()
            } else if (event.organizer.isEmpty()) {
                Snackbar.make(it, R.string.hint_event_organizer, Snackbar.LENGTH_LONG)
                    .show()
            } else if (event.date.year == 0) {
                Snackbar.make(it, R.string.hint_event_date, Snackbar.LENGTH_LONG)
                    .show()
    ...
    override fun onTimeSet(view: TimePicker?, hourOfDay: Int, minute: Int) {
        ...
        binding.showDate.text =
            "$saveYear - $saveMonth - $saveDayofMonth, ${format.format(saveHour)} : ${
                format.format(saveMinute)}"
    }
